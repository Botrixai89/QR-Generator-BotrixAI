...
export const addBotrixLogoToQR = (svg: SVGElement) => {
  ...
  const logoSize = Math.max(36, Math.min(80, baseSize * 0.18)) // 18% of QR size, min 36px, max 80px
  ...
  logoImage.setAttribute('width', (logoSize - 2).toString())
  logoImage.setAttribute('height', (logoSize - 2).toString())
